...
def main(args):
    ...
    if args.MACHINE is None:
        ...
        main_root = args.ROOT+'/6db/'+args.MACHINE
        ...
        #         com.visualize(normal,anomaly,output_path,CHECK_POINT,section,domain) #AUC,異常度の可視化
        for id_ in ID_:
            # train_path,normal_path,anomaly_path = com.make_path_list(main_root,id_,args.SEED)
            train_path,valid_path,normal_path,anomaly_path = com.make_path_list(file_root,id_,args.SEED)
            train_path = train_path + valid_path
            test = com.testset(train_path,args.FRAME_LENGTH,args.FRAME_SHIFT_LENGTH,args.BINS,output_path,CHECK_POINT)
            if id_ == 'id_00':
            ...
            length = int(len(test)//len(train_path))
            ...
            for i in tqdm(range(len(train_path))):
                ...
                # mi[i*length:(i+1)*length] = f.to('cpu').detach().numpy().copy()
            ...
            plt.close()
            com.visualize(score[:len(normal_path)],score[len(normal_path):],output_path,CHECK_POINT,section,domain,id_)
            np.random.shuffle(anomaly_middle)
            np.random.shuffle(normal_middle)
            np.random.shuffle(mi)
            anomaly_tsne = anomaly_middle[:int(len(anomaly_middle)//10)]
            normal_tsne = normal_middle[:int(len(anomaly_middle)//10)]
            mi_tsne = mi[:int(len(mi)//10)]
            index = [0,len(mi_tsne),len(mi_tsne)+len(anomaly_tsne),len(mi_tsne)+len(anomaly_tsne)+len(normal_tsne)]

            tsne_x = np.concatenate([mi_tsne,normal_tsne,anomaly_tsne])
            color_ = ['b','g','r']
            label_ = ['train(normal)','test(normal)','test(anomaly)']

            from sklearn.manifold import TSNE
            tsne = TSNE(n_components=2, random_state = 0)
            emb = tsne.fit_transform(tsne_x)
            for i in range(3):
                plt.scatter(emb[index[i]:index[i+1],0],emb[index[i]:index[i+1],1],c=color_[i],alpha=0.5,s=1,label=label_[i])
            plt.legend()
            plt.title('tsne')
            plt.savefig(output_path+"/{}/{}_tsne".format(CHECK_POINT,id_))
            plt.close()
