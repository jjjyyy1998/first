...
def main(args):
    ...
    if args.MACHINE is None:
        ...
        output_path = args.OUTPUT_PATH+args.MACHINE +'/' 
        # model_weights=output_path+'/{}/UNet.h5'.format(CHECK_POINT)
        # model = models.UNet() #モデル作成
        # model = models.DeepSVDD(model,248)
        ...
        for id_ in ID_:
            ...
            length = int((len(test)//len(train_path))/2)
            ...
            for i in tqdm(range(len(train_path))):
                ...
                # inputs = test[i*length:(i+1)*length].to(device)
                    inputs = test[(2*i+j)*length+j:(2*i+j+1)*length].to(device)
                    copy_ = inputs.clone()
                    # copy_ = inputs.clone()
                    # out,hidden_half,_,x1,x2 = model([inputs,copy_])
                    out,hidden_half,_ = model([inputs,copy_])
                    
                    # mi[i*length:(i+1)*length] = hidden_half[0].to('cpu').detach().numpy().copy()
                    mi[(2*i+j)*length+j:(2*i+j+1)*length] = hidden_half[0].to('cpu').detach().numpy().copy()
            ...
            score =[]
            for i in tqdm(range(len(normal_path)*2)):
                score.append(np.mean(mahala_result[2*length*i:(i+1)*length*2]))
            plt.plot(mahala_result)
            plt.savefig(output_path+"/{}/{}_middle".format(CHECK_POINT,id_))
            plt.close()
            com.visualize(score[:len(normal_path)],score[len(normal_path):],output_path,CHECK_POINT,section,domain,id_)
            ...
            #     copy_ = inputs.clone()
            #     out,hidden_half,_ = model([inputs,copy_])
            #     mi[i*length:(i+1)*length] = _[0].to('cpu').detach().numpy().copy()
            # mu_mat = np.mean(mi,axis=0)
            # # cov = np.cov(mi.T)
            # # cov = np.linalg.pinv(cov)
            # cov = LedoitWolf().fit(mi)
            ...
            # normal=[]
            # anomaly=[]
            # normal_middle = com.anomaly_score2(mean_,std_,normal_path,output_path,CHECK_POINT,normal,model,args.FRAME_SHIFT_LENGTH,args.FRAME_LENGTH,args.BINS,args.PATCH_NUM,MASK_NUM,args.GIF,device) #正常データの異常度算出
            # anomaly_middle = com.anomaly_score2(mean_,std_,anomaly_path,output_path,CHECK_POINT,anomaly,model,args.FRAME_SHIFT_LENGTH,args.FRAME_LENGTH,args.BINS,args.PATCH_NUM,MASK_NUM,args.GIF,device) #異常データの異常度算出
            # section = None
            # domain = None
            # com.visualize(normal,anomaly,output_path,CHECK_POINT,section,domain,id_) #AUC,異常度の可視化
            # abnormality = np.concatenate([normal_middle,anomaly_middle],axis=0)
